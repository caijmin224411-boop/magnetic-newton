...
建议在每个摆球正面贴上颜色明显的小圆点，标定时只框住这个色点。不同摆可以使用不同颜色，也可以使用同一种颜色；程序会结合色点颜色、悬点位置和摆长约束来减少串扰。背景尽量单一，摄像头固定并与摆动平面正对。做混沌分析时，优先使用 `angle_deg` 或 `angle_rad` 时间序列，再计算相空间、庞加莱截面、相关维数或 Lyapunov 指数。
